...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    public void createNewTasks() {
        ...
        // Теперь методы возвращают объекты напрямую, а не Optional
        ...
        assertNotNull(taskManager.getTask(3), "Такого объекта не существует");
        ...
        assertNotNull(taskManager.getEpic(4), "Такого объекта не существует");
        ...
        assertNotNull(taskManager.getSubtask(5), "Такого объекта не существует");
    ...
    public void testDeleteTaskById() {
        ...
        assertThrows(NotFoundException.class, () -> taskManager.getTask(0), "Задача не удалена, должно быть исключение");

        // Удаляем подзадачу
        taskManager.deleteTasksById(TaskType.SUBTASK, 2);
        assertThrows(NotFoundException.class, () -> taskManager.getSubtask(2), "Подзадача не удалена, должно быть исключение");

        // Удаляем эпик (должны удалиться и его подзадачи)
        taskManager.deleteTasksById(TaskType.EPIC, 1);
        assertThrows(NotFoundException.class, () -> taskManager.getEpic(1), "Эпик не удален, должно быть исключение");
    }
    ...
    public void testIntersections() {
        ...
        taskManager.createTask(TaskType.SUBTASK, "Подзадача 1", "Описание", 1,
                TaskProgress.NEW, "2005-12-14T00:00", 120, "2005-12-14T02:00");
        assertNotNull(taskManager.getSubtask(2), "Пересечение с существующей задачей, объект должен быть равен null");

        taskManager.createTask(TaskType.SUBTASK, "Подзадача 2", "Описание", 1,
                TaskProgress.NEW, "2005-12-15T00:00", 120, "2005-12-15T02:00");
        assertNotNull(taskManager.getSubtask(3), "Объекты пересекаются, не должен быть равен null");

        // Тестируем случаи, когда задачи не должны создаваться из-за пересечений
        // Предполагается, что ваш менеджер задач выбрасывает исключение при пересечении
        assertThrows(ManagerSaveException.class, () ->
                        taskManager.createTask(TaskType.TASK, "Задача 5", "Описание", 0,
                                TaskProgress.NEW, "2005-12-12T00:00", 60, "2005-12-12T23:00"),
                "Задача с пересечением времени должна вызывать исключение");

        // Эта задача должна создаться нормально (нет пересечения)
        taskManager.createTask(TaskType.TASK, "Задача 6", "Описание", 0,
                TaskProgress.NEW, "2005-12-12T03:00", 60, "2005-12-12T04:00");
        assertNotNull(taskManager.getTask(4), "Задача после существующей, не должна быть null");

        taskManager.createTask(TaskType.EPIC, "Эпик 2", "Описание", 0,
                TaskProgress.NEW, "2005-12-12T00:00", 120, "2005-12-12T02:00");
        assertNotNull(taskManager.getEpic(5), "Эпик может быть создан в любое время, не должен быть null");
